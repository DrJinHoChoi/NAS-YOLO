...
Modern detectors---YOLOv8~\cite{yolov8}, YOLOv9~\cite{yolov9}, RT-DETR~\cite{rtdetr}---achieve strong accuracy on clean benchmarks while maintaining real-time throughput on edge devices.
...
\begin{itemize}[leftmargin=1.5em,topsep=2pt,itemsep=1pt]
  \item Competitive clean mAP with significant mAP-C improvement (\textbf{+X.X} over YOLOv8).
  \item \textbf{X\% higher BSI} than all baselines across corruption types.
  \item Real-time performance on edge GPUs ($>$100 FPS on NVIDIA Jetson).
\end{itemize}
...
The YOLO family~\cite{yolov1,yolov3,yolov4,yolov5,yolov7,yolov8,yolov9,yolov10} has driven real-time detection forward through architectural innovations: CSP connections~\cite{cspnet}, path aggregation networks~\cite{panet}, anchor-free heads~\cite{fcos,yolox}, and efficient backbones.
...
\begin{table}[t]
  ...
  \begin{tabular}{l|ccc|cc|cc}
    ...
    YOLOv10-n & 2.3 & 6.7 & 390 & 38.5 & 53.8 & -- & -- \\
    Gold-YOLO-n & 5.6 & 12.1 & 310 & 39.9 & 55.6 & -- & -- \\
    \ours{}-n & 3.5 & 9.2 & 330 & -- & -- & -- & -- \\
    \midrule
    ...
    Gold-YOLO-s & 21.5 & 46.0 & 250 & 45.4 & 62.5 & -- & -- \\
    \ours{}-s & 8.5 & 13.0 & 270 & -- & -- & -- & -- \\
    \bottomrule
  \end{tabular}
\end{table}

% Placeholder for results discussion
\textit{Results will be populated upon completion of experiments.}
...
\begin{table}[t]
  \centering
  \caption{\textbf{Per-corruption category mAP-C} on COCO-C validation set. Values are mAP averaged across 5 severity levels.}
...
Removing the Selective State Space Module eliminates temporal reasoning, reducing BSI by --\% and mAP-C by --\%.
Clean mAP is minimally affected, confirming that S3M primarily improves robustness and stability.

\paragraph{Effect of Noise-Aware Gating (C2).}
Without the gate, the S3M always blends temporal and current features equally.
This hurts clean mAP (temporal state slightly blurs clean features) while providing less corruption benefit.
The gate enables \textit{no-cost-on-clean} operation.

\paragraph{Effect of TCL.}
Removing TCL retains the architectural benefits of S3M but loses explicit temporal smoothness regularization.
BSI drops by --\%, confirming TCL's role in training stability-aware representations.
...
The NASM adds only --\% parameters and --ms latency to the base detector, validating our edge-friendly design goal (C3).
...
Through Selective State Space temporal modeling (C1), noise-conditioned gating (C2), and the novel Box Stability Index metric (C4), we demonstrated that temporal reasoning is the missing ingredient for robust, stable detection under real-world visual corruptions---achievable with minimal computational overhead (C3).
